...
void write_valid_information()
{
    ...
    printf("valid_num_trees: %lu [size %u]\n", valid_num_trees, sizeof(vertex_id_t));
    assert(fwrite(valid_weight_trees, sizeof(weight_t), valid_num_trees, F) == valid_num_trees);
    for (int i = 0; i < valid_num_trees; i++) {
        printf("valid_weight_trees[%d]: %lf [size %u]\n", i,
            valid_weight_trees[i], sizeof(weight_t));
    }
    fclose(F);
}
